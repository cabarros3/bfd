...
class Produto {
  ...
  adicionarEstoque(qtd: number): void{
    this.quantidade += qtd
    console.log(`A nova quantidade do produto ${this.nome} é ${this.quantidade}`)
  }
...
//     const produto = estoque.getListaProduto
//   }
// }
...
class Estoque {
  ...
  calcularValorTotal(): void{
    let calculaTotal = 0
    for(let i: number = 0; i < this.listaProdutos.length; i++){
      calculaTotal += this.listaProdutos[i].getPreco * this.listaProdutos[i].getQuantidade
    }
